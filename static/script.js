...
// 5.4 Função carregarOrdens() - GET /ordens
async function carregarOrdens() {
    ...
    // Exibe indicador de carregamento e esconde a tabela
    loading.classList.remove('oculto');
    tabela.classList.add('oculto');
    semDados.classList.add('oculto');

    try {
        // Faz a requisição GET /ordens
        const resposta = await fetch(`${API_URL}/ordens`);
        
        // Converte a resposta (texto JSON) em array JavaScript
        const ordens = await resposta.json();

        // Esconde o loading
        loading.classList.add('oculto');

        // Se não há ordens, mostra mensagem e sai da função
        if (ordens.length === 0) {
        ...
        exibirMensagem('Erro ao conectar com a API. O servidor está rodando?', 'erro');
    }
}

// Retorna o HTML do badge colorido conforme o status
...
async function excluirOrdem(id) {
    // Exibe um alerta de confirmação do navegador
    const confirmado = window.confirm(`Tem certeza que deseja excluir a Ordem #${id}? Esta acao e permanente.`);
    
    if (!confirmado) return; // Se o usuário clicar em "Cancelar", para a execução aqui

    try {
        const resposta = await fetch(`${API_URL}/ordens/${id}`, {
            method: 'DELETE' // O método DELETE geralmente não precisa de 'body'
        });
        ...
        if (resposta.ok) {
            ...
            // Verifica se a tabela ficou vazia para mostrar a mensagem de "Nenhuma ordem"
            const corpo = document.getElementById('corpo-tabela');
            ...
            await verificarStatus(); // Atualiza o contador de ordens
        } else {
...
function exibirMensagem(texto, tipo) {
    ...
    // Define a classe CSS baseada no tipo (ex: 'mensagem sucesso' ou 'mensagem erro')
    div.className = `mensagem ${tipo}`; 
    
    // Torna a mensagem visível
    div.classList.remove('oculto');

    // Timer: Esconde a mensagem automaticamente após 4 segundos (4000ms)
    setTimeout(() => {
...
window.onload = async function() {
    // Busca o status do servidor (Online/Offline)
    await verificarStatus();
    // Busca a lista de ordens do banco de dados
    await carregarOrdens();
};